...
class ServiceDispatcher {
  ...
  private mainCppcData:string[] = [];
	...
	constructor() {
		...
		this.connection.onCodeAction((params: CodeActionParams) => this.onCodeAction(params));
  ...
  private onDidChangeContent (change: TextDocumentChangeEvent<TextDocument>): void{
    ...
    try {
      ...
      if(complitionPosition.context?.triggerKind === 2 || word.includes('.')) // trigged by '.'
      {
        ...
        if(componentId === 'JSON')
        {
          ...
          if(!match) return this.getPropsFromProp(text, complitionPosition.position.line, word);
  ...
  private getProps(component:string): any {
    ...
    if(defaultComp.inherit) return Object.assign({}, props, this.getProps(defaultComp.inherit));
    return props;
  }
  ...
  private getPropsFromProp(text: string, line: number, property = ""): CompletionItem[] {
    let prop: string[] = [];
    let itens: CompletionItem[] = [];
  ...
  private getDefaultComponents(): CompletionItem[] {
	...
	private onCodeAction(params: CodeActionParams):HandlerResult<(Command | CodeAction)[] | null | undefined, void> {
    if(!params.context.diagnostics.length) return;
    const doc = this.documents.get(params.textDocument.uri);
    const codeAction = params.context.diagnostics[0].data as string || params.context.diagnostics[0].message.split(' ')[0] || "";
    const actions: CodeAction[] = [this.createImport(doc!, codeAction)];
		return actions;
	}
